...
from dialated_semantic_segmentation import Model
...
def train(params):
    ...
    learning_rate = tf.train.exponential_decay(params['init_learning_rate'], iters, 50000, 0.5)
    ...
    tf.losses.add_loss(loss_func)
    model.loss_func = tf.losses.get_total_loss()
